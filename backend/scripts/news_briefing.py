...
async def save_articles_to_doc(session: AsyncSession, articles: list[dict], source: RssSource) -> int:
    ...
    for article in articles:
        ...
        # 生成唯一标识
        article_uuid = generate_article_uuid(article["link"], article["title"])
...
async def fetch_all_sources(session: AsyncSession) -> int:
    ...
    for source in sources:
        ...
        try:
            ...
            saved = await save_articles_to_doc(session, articles, source)
            total_saved += saved
...
async def main():
    ...
    async with async_db_session() as session:
        # 1. 初始化RSS订阅源
        log.info("\n[步骤1] 检查并初始化RSS订阅源")
        await init_rss_sources(session)

        # 2. 从RSS源抓取文章
        log.info("\n[步骤2] 从RSS源抓取文章")
        total_articles = await fetch_all_sources(session)
