...
async def process_with_llm(config, input_text, debug=False):
    ...
    # 动态修改提示词，添加自定义实体类型（去重）
    builtin_types = {"人物", "组织", "地点", "事件", "邮箱"}
    entity_types = config.get("entity_types")
    if entity_types and isinstance(entity_types, list):
        extra = [t for t in entity_types if t not in builtin_types]
        extra_str = "、" + "、".join(extra) if extra else ""
    else:
        extra_str = ""
    user_prompt = user_prompt.replace("{extra_entity_types}", extra_str)
