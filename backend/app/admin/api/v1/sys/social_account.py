...
async def get_pagination_social_account(db: CurrentSession) -> ResponseModel:
    social_account_select = await social_account_service.get_select()
    page_data = await paging_data(db, social_account_select, GetSocialAccountListDetails)
...
async def create_social_account(obj: CreateSocialAccountParam) -> ResponseModel:
    await social_account_service.create(obj=obj)
...
async def update_social_account(pk: Annotated[int, Path(...)], obj: UpdateSocialAccountParam) -> ResponseModel:
    count = await social_account_service.update(pk=pk, obj=obj)
    if count > 0:
        return response_base.success()
...
async def delete_social_account(pk: Annotated[list[int], Query(...)]) -> ResponseModel:
    count = await social_account_service.delete(pk=pk)
    if count > 0:
        return response_base.success()
    return response_base.fail()