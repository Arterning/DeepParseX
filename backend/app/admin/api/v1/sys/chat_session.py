...
from backend.app.admin.schema.chat_session import CreateChatSessionParam, GetChatSessionDetails, GetChatSessionListDetails, UpdateChatSessionParam
...
async def get_chat_session(pk: Annotated[int, Path(...)]) -> ResponseModel:
    chat_session = await chat_session_service.get(pk=pk)
    data = GetChatSessionListDetails(**select_as_dict(chat_session))
...
async def get_pagination_chat_session(db: CurrentSession) -> ResponseModel:
    chat_session_select = await chat_session_service.get_select()
...
async def create_chat_session(obj: CreateChatSessionParam) -> ResponseModel:
    await chat_session_service.create(obj=obj)
